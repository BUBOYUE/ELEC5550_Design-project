 ...
 * File:    esp32s3_usb_passthrough.ino
 * Authors : UWA ELEC5550 Team 13 (merged by Boyue BU)
 * Date:    2025-10-15
 * 
 * Summary:
 *   Unified sketch that lets one ESP32‑S3 board act as a USB Host (HID + MSC)
 *   and transparently bridge traffic to a peer ESP32‑S3 over UART using a
 *   framed protocol with CRC and optional FEC (Reed–Solomon). The peer may act
 *   as a USB Device endpoint (HID/MSC). Mode selection is done via GPIO5.
 *
 * Context:
 *   University of Western Australia (UWA) – ELEC5550 Design Project, Group 13.
 *   This file consolidates the previously separate HID and MSC passthrough
 *   sketches and adds UART framing, task scheduling, and logging for a laser
 *   data link.
 *
 * Key Features:
 *   - TinyUSB Host: HID (keyboard/mouse) + MSC (U‑Disk) handling on S3‑A
 *   - UART framing: Type/Len/Payload + CRC, optional RS FEC (payload ≤ 224 B)
 *   - FreeRTOS tasks: USB events, MSC app queue, UART RX/TX, HID callbacks
 *   - Zero‑block optimization for MSC reads (U_EMPTY), write acknowledgement
 *   - Dual‑mode: Host/Device decided by GPIO5 (HIGH=Host, LOW=Device)
 *
 * Build/Run Notes:
 *   - Platform: Arduino‑ESP32 for ESP32‑S3
 *   - Serial monitor: 460800 bps (logging and diagnostics)
 *   - Power/role: Set board role with the on‑board switch tied to GPIO5
 *
 * References:
 *   - ESP‑IDF examples: HID Host (hid_host) and MSC Host (msc_host)
 *   - TinyUSB stack and ESP32‑S3 USB Host driver
 *
 * Disclaimer:
 *   Educational code produced by UWA student team; provided "as is" without any
 *   warranty. Review licensing before redistribution.
 */
...
#include "uart.h"   // Provides: uart2_init(), send_frame(...)
...
static void hid_host_mouse_report_callback(const uint8_t *const data,
  ...
  if (g_mouse_ctx.inited && data && length > 0) {
    ...
    if (pr == hid::ERR_SUCCESS) {
      ...
      // Collect relative axes: X/Y and wheel
      ...
      // Send 4-byte payload: [buttons, dx, dy, wheel]
      uint8_t payload[4];
      payload[0] = buttons;
  ...
  // Fallback: original heuristic processing, compatible with unparsed cases
  ...
  // Pack useful 4-byte payload: [buttons(2:middle 1:right 0:left), dx, dy, wheel]
  uint8_t payload[4];
  payload[0] = buttons;
  payload[1] = (uint8_t)dx;
  payload[2] = (uint8_t)dy;
...
void hid_host_interface_callback(hid_host_device_handle_t hid_device_handle,
  ...
  case HID_HOST_INTERFACE_EVENT_DISCONNECTED:
    ...
    // Reset parser when mouse is disconnected
...
// true  -> run app_main() (host/passthrough)
// false -> run device mode (HID/MSC)
...
void setup() {
  // Serial and mode selection pin
  Serial.begin(460800);
  delay(200);
  pinMode(PIN_MODE_SEL, INPUT_PULLUP);   // External pull-down = device mode; floating/pull-up = host mode

  // UART initialization (both ends need it)
  uart2_init();

  // Read selection (power-on determination; if runtime switching needed, extend in loop)
  g_run_host = (digitalRead(PIN_MODE_SEL) == HIGH);

  if (g_run_host) {
    // ---------- Host / Passthrough branch ----------
    Serial.println("ESP32-A: HID+MSC passthrough (HOST) selected by GPIO5=HIGH");
    // Only start app_main once, internally creates FreeRTOS tasks, usb_host_install etc.
